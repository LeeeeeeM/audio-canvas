...
- 支持在线音频链接与本地文件两种输入源，统一通过 Web Audio API 播放与分析。
- 提供基础播放控制与至少一种可视化效果（频谱柱状图），为后续扩展保留接口。

## 2. 功能范围
- 音源管理：文本框输入 HTTP(S) 链接、文件选择器上传本地音频。
...
## 4. 组件拆分
- `SourceSelector`：输入远程 URL、校验格式；本地文件选择并回传 `File`。
...
## 5. 数据流与状态
- 顶层页面维护当前音源描述对象：`{ type: 'url' | 'file', url?: string, file?: File }`。
